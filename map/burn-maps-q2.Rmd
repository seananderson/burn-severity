...
```{r fit, results='hide'}
library(here)
source(here("zoib-functions.R"))
# data <- readr::read_rds("RedRocks_coords_RdNBR_BL.rds")
data <- readr::read_csv(here("map/RedRocks_data_Fig6.csv"))
...
sm <- rstan::stan_model(here("zoib1re.stan"))
...
out_q2 <- plyr::mlply(.data = vars, function(x, y) {
  ...
  m <- fit_model(d, model = sm,
    ...
    iter = 2000L, chains = 4L, adapt_delta = 0.98, seed = 42)
...
readr::write_rds(xx, here("map/RedRocks_predictions_Q2.rds"))
```
...
checking_square <- geom_polygon(data = data.frame(x = c(x_, x_ + size_, x_ + size_, x_),
    y = c(y_, y_, y_ + size_, y_ + size_), response = "Charred surface cover"), aes_string(x = "x", y = "y"),
...
ggsave(here("map/map-cred2-Q2.pdf"), width = 3.9, height = 7.9)
...
select(xx, x, y, response, pp_est, p0_est, p1_est, q0.50) %>%
  ...
  geom_text(data = lets2, aes(label = panel, x = x, y = y), 
    inherit.aes = FALSE, col = "grey30")
ggsave(here("map/map-components-Q2.pdf"), width = 4, height = 7.5)
```
