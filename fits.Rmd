...
Each thin line is the production for individual fire (incorporates the random intercepts). The thick line is the median main effect prediction in the shaded region shows the 95% credible interval.
...
Now let's try a model that includes other covariates and their interaction with RdNBR. 
...
I haven't done it here yet, but another way to represent these effects would be to show their effect on the predictions holding the other variables after their mean values. It would look like the first plot in this document. Otherwise, the coefficients themselves have to be kept separate. 
