...
fit_model <- function(d, model, predictors = "xscaled", iter = 1000L, chains = 4L,
  adapt_delta = 0.8) {
  library(rstan)
...
make_predictions <- function(d, f, model, Npred = 200L) {
  d_pred <- expand.grid(
...
# dd <- read_csv("ALL_SHOBS_2010_2014_PLOTS_MASTER_w_CO_FINAL_20171029.csv")
library(rstan)
...
m <- fit_model(d, model = sm,
  predictors = c("xscaled", "northing*xscaled", "elev*xscaled", 
    "slope*xscaled", "heatload*xscaled", "aspect*xscaled"))
```

```{r plot-interactions, cache=TRUE, dependson=c("interactions", "functions")}
terms <- tibble(
...
b %>% 
  ...
  ggplot(aes(y = estimate, x = name, colour = model_full, shape = interaction)) +
  ...
  labs(colour = "Model", shape = "Interaction", y = "Coefficient")
```
