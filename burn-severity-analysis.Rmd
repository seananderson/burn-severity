  ...
  out_cv <- plyr::mlply(.data = vars, function(x, y) { # or map2() without parallel
    ...
    m <- map(fires, function(x) {
      fit_model(d = d[d$group != x, , drop = FALSE], model = sm,
        predictors = "xscaled", iter = 1000L, chains = 1L, log_lik = FALSE)
    })
...
d <- get_dat(Firemort.BA.p, RBR, file = "data/RockiesBurnSev_clean_20180124.csv")
...
m <- fit_model_oib(dd, sm_oib, predictors = x_vars)
```
