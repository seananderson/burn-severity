...
inter_vars_long <- c("QMD (live at fire)", "QMD", "Stand basal area", "Stand density", 
  "Pre-fire beetle\noutbreak severity")
vars_i <- seq_along(inter_vars)
...
for (j in response_j) {
  for (i in vars_i) {
    k <- k + 1
    q3_inter_plots[[k]] <- 
      plot_interaction(out_q3[[j]], int_var = inter_vars[i], int_lab = paste0("\n", inter_vars_long[i]), 
      xlab = gsub("_BL", "", "RdNBR_BL"), col = cols, quant = quants, 
        title = var_names$new.names[var_names$y == vars$y[[j]]], 
        type = "oib")
  }
}
pdf("figs/q3-interaction-ribbons.pdf", width = 13, height = 10)
do.call("grid.arrange", c(q3_inter_plots, nrow = length(response_j)))
dev.off()
```
