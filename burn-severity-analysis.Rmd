...
ggsave("figs/roc-example.pdf", width = 5.5, height = 5)
```
...
pdf("figs/q2-coefs.pdf", width = 9, height = 8)
do.call("grid.arrange", c(p_q2, ncol = nCol))
dev.off()
```
...
g <- ggplot(q2_inter_plots_df, aes(x, est, ymin = lwr, ymax = upr, group = level,
  ...
  theme(legend.justification = c(1, 0), legend.position = c(0.2, 0.8), 
    panel.spacing = unit(-0.05, "lines"), legend.title = element_text(size = rel(0.8))) +
  xlab("RdNBR")

pdf("figs/q2-interaction-ribbons.pdf", width = 8, height = 5.5)
print(g)
dev.off()
```
...
aucs %>%
  ...
  theme(strip.text = element_text(size = rel(0.87))) +
  coord_cartesian(xlim = c(0, 0.99), expand = 0)
ggsave("figs/auc-q1-vs-q2.pdf", width = 8.2, height = 2.5)
```
