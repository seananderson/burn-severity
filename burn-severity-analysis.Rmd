...
out_q3 <- plyr::mlply(.data = vars, function(x, y) { # or map2() without parallel
  ...
  m <- fit_model_oib(dd, sm_oib, predictors = x_vars, iter = 2000, chains = 4)
...
plot_interaction <- function(obj, int_var, int_lab, xlab, quant = c(0.1, 0.9)) {
  ...
  newdata2 <- newdata
  newdata2[,int_var] <- quantile(obj$d$ba_ha, quant[[2]])
  newdata2$level <- as.character(quant[[2]])
  ...
  # cols <- RColorBrewer::brewer.pal(3, "Set2")[c(2, 3)]
  names(cols) <- c("0.1", "0.9")
