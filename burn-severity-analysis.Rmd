...
```{r}
temp_dat <- aucs_q1 %>% filter(x %in% c("dNBR_BL", "RBR_BL", "RdNBR_BL")) %>%
  mutate(x = gsub("_BL", "", x)) %>%
...
lets <- data.frame(response = response_order,
  ...
  stringsAsFactors = FALSE, new.names = unique(temp_dat$new.names))
...
newnames_df <- data.frame(
  name = c(" RdNBR_BL", " RdNBR_BL:heatload", " RdNBR_BL:lat", 
    " RdNBR_BL:prenbr", 
    " RdNBR_BL:slope", "heatload", "lat", "prenbr", "slope"),
  new.names = c(" RdNBR", " RdNBR x heat load", " RdNBR x latitude", 
...
gg <- ggplot(p_q2, aes(y = estimate, x = xvar,
...
# 
# pdf("figs/q2-coefs.pdf", width = 9, height = 8, units = "in", res = 200)
# do.call("grid.arrange", c(p_q2, ncol = nCol))
# dev.off()
```
...
# pdf("figs/q2-interaction-ribbons.pdf", width = 13, height = 10, dpi = 200)
...
out_q3_plot <- out_q3
# var_names$clean_orig <- gsub("\\.", "_", var_names$y)
# var_names$clean_orig <- tolower(var_names$clean)


for (i in seq_along(out_q3_plot)) {
  out_q3_plot[[i]]$y <- var_names$new.names[var_names$y == out_q3_plot[[i]]$y]
  #   names(out_q3_plot[[i]]$m$stan_dat$Xp_ij)[j] <- var_names$new.names[var_names$y == names(out_q3_plot[[i]]$m$stan_dat$Xp_ij)[j]]
  # }
  # for (j in seq_along(var_names$y)) {
  #   colnames(out_q3_plot[[i]]$m$stan_dat$Xp_ij) <- 
  #     stringr::str_replace(colnames(out_q3_plot[[i]]$m$stan_dat$Xp_ij), var_names$y[j], var_names$new.names[j])
  # }
}

newnames_df <- data.frame(
  name = c(" RdNBR_BL", "qmd_all", "ba_ha", "stems_ha", "redgraygreenstage_ba_p", 
    " RdNBR_BL:qmd_all", " RdNBR_BL:ba_ha", " RdNBR_BL:stems_ha", 
    " RdNBR_BL:redgraygreenstage_ba_p"),
  new.names = c(" RdNBR", "QMD", "Stand basal area", "Stand density", "Pre-fire beetle outbreak severity", 
...
gg <- ggplot(p_q3, aes(y = estimate, x = xvar,
  colour = model_full, shape = interaction)) +
  geom_hline(yintercept = 0, lty = 2, col = "grey55") +
  geom_linerange(aes(ymin = conf.low.25, ymax = conf.high.75),
    position = position_dodge(width = 0.4), size = 0.85) +
...
# p_q3 <- lapply(out_q3_plot, function(x) plot_zoib_coefs(x, oib = TRUE))
# library(gridExtra)
# n <- length(p_q3)
# nCol <- floor(sqrt(n))
# do.call("grid.arrange", c(p_q3, ncol = nCol))
# 
# pdf("figs/q3-coefs.pdf", width = 10, height = 8)
# do.call("grid.arrange", c(p_q3, ncol = nCol))
# dev.off()
```
