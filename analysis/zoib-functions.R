...
get_dat <- function(response, main_predictor,
  ...
  d <- suppressMessages(readr::read_csv(file))
  ...
  d <- select(
    d, !!response, !!main_predictor, FIRE,
  ...
  if (scale_other_predictors) {
    d <- dplyr::mutate_at(
      d,
      c(
        "lat", "heatload", "slope", "prenbr", "qmd_laf", "qmd_all",
...
make_pred_plot <- function(pred_df, raw_data, re = TRUE) {
  g <- ggplot(pred_df, aes(x, est, ymin = lwr, ymax = upr)) +
    geom_point(
      data = raw_data, aes(x, y, colour = group),
